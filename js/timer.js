...
function GameTimer(d) {
    ...
    this.reset = function () {
        ...
        this.setState("reset");
        if (this.goldCounter > 0) {
            if (window.confirm("Would you like to save your gold splits?")) {
                this.saveGoldSplit();
            }
        } // Wow
        this.currentSplit = 1;
    ...
    this.updateSplit = function (splittime) {
        var timerText = document.getElementById("split" + this.currentSplit), currentSegment = splittime - this.getSegmentTime(), bestSegment = splitsObject[this.currentSplit][2], prevSplit = document.getElementById("prevsplit"), prevText = document.getElementById("prevtext");
        // Double Tap Prevention
        if (currentSegment < 300) {
            return false;
        }
        ...
        if (this.totalSplits !== this.currentSplit) {
            ...
            if (this.getTotalTime() > this.getSegmentTime() || this.getTotalTime() === 0 || splitsObject[this.totalSplits][1] === 0) {
    ...
    this.skipSplit = function () {
        if (this.currentSplit === this.totalSplits || this.currently === "stop") {
            return false;
        } // can't skip last split
        splitsObject[this.currentSplit][3] = 0;
        document.getElementById("difference" + this.currentSplit).textContent = '-';
    ...
    this.startSplits = function () {
        ...
        if (Object.keys(splitsList).length === 0) {
            ...
            else if (localStorage.splitsDefault) {
    ...
    this.splitSelector = function () {
        if (this.currently === 'play' || this.currently === 'pause' || this.editorEnabled === true) {
            return false;
        }
        this.setState("menu");
    ...
    this.wsplitExport = function () {
        ...
        var splitSplits = "", splitIcons = "", addtime = 0;
        for (var step = 1; step <= this.totalSplits; step++) {
    ...
    this.deleteSplitFile = function (id) {
        if (confirm("WARNING!:This will irrevocably delete the selected splits, Are you SURE you want to continue?")) {
        }
        else {
            /* OK */
            return false;
    ...
    this.saveSplits = function () {
        if (this.disableControls === true || this.currently === 'play') {
            return false;
        }
    ...
    this.setStyle = function (currentState) {
        ...
        if (currentState === 'stop') {
            for (var step = 1; step <= this.totalSplits; step++) {
                var difference = document.getElementById("difference" + step), row = document.getElementById("row" + step);
    ...
    this.setSegmentColor = function (currentSegment) {
        var timerText = document.getElementById("split" + this.currentSplit), prevSplit = document.getElementById("prevsplit"), pbSegment = splitsObject[this.currentSplit][1], bestSegment = splitsObject[this.currentSplit][2];
    ...
    var self = this, d = d || {}; // I really don't know about this.
    this.timer = { start: 0, now: 0, realtime: 0 };
    this.elements = {
